...
router.get('/', async (req, res) => {
  try {
    let data = await db.find('Ingredients')
    res.send(data)
  }
  catch (err) {
...
router.get('/:id', validateDataId, async (req, res) => {
  try {
    let data = await db.findById(req.data.id, 'Ingredients')
    res.send(data)
  }
  catch (err) {
    res.status(500).send(err.message)
...
router.put('/:id', idBodyCheck, async (req, res) => {
  try {
    let data = await db.update(req.data.id, req.body, 'Ingredients')
    res.json(data)
  }
  catch (err) {
    res.status(500).json(err.message)
...
router.delete('/:id', validateDataId, async (req, res) => {
  try {
    let data = await remove(req.data.id, 'Ingredients')
    res.json(data)
  }
  catch (err) {
    res.status(500).json(err.message)
...
async function validateDataId(req, res, next) {
  try {
    let data = await db.findById(req.params.id, 'Ingredients')
    if (data) {
      req.data = data
      next()
    } else {
